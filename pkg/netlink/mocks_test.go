// Code generated by mockery; DO NOT EDIT.
// github.com/vektra/mockery
// template: testify

package netlink

import (
	"net"

	mock "github.com/stretchr/testify/mock"
)

// NewMockWatcher creates a new instance of MockWatcher. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
// The first argument is typically a *testing.T value.
func NewMockWatcher(t interface {
	mock.TestingT
	Cleanup(func())
}) *MockWatcher {
	mock := &MockWatcher{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}

// MockWatcher is an autogenerated mock type for the Watcher type
type MockWatcher struct {
	mock.Mock
}

type MockWatcher_Expecter struct {
	mock *mock.Mock
}

func (_m *MockWatcher) EXPECT() *MockWatcher_Expecter {
	return &MockWatcher_Expecter{mock: &_m.Mock}
}

// Watch provides a mock function for the type MockWatcher
func (_mock *MockWatcher) Watch() <-chan net.IP {
	ret := _mock.Called()

	if len(ret) == 0 {
		panic("no return value specified for Watch")
	}

	var r0 <-chan net.IP
	if returnFunc, ok := ret.Get(0).(func() <-chan net.IP); ok {
		r0 = returnFunc()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(<-chan net.IP)
		}
	}
	return r0
}

// MockWatcher_Watch_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'Watch'
type MockWatcher_Watch_Call struct {
	*mock.Call
}

// Watch is a helper method to define mock.On call
func (_e *MockWatcher_Expecter) Watch() *MockWatcher_Watch_Call {
	return &MockWatcher_Watch_Call{Call: _e.mock.On("Watch")}
}

func (_c *MockWatcher_Watch_Call) Run(run func()) *MockWatcher_Watch_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run()
	})
	return _c
}

func (_c *MockWatcher_Watch_Call) Return(updates <-chan net.IP) *MockWatcher_Watch_Call {
	_c.Call.Return(updates)
	return _c
}

func (_c *MockWatcher_Watch_Call) RunAndReturn(run func() <-chan net.IP) *MockWatcher_Watch_Call {
	_c.Call.Return(run)
	return _c
}
